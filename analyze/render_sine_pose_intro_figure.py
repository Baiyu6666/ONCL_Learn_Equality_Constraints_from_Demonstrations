...
from analyze.render_sine_pose_demonstrations import (  # noqa: E402
    DATASET_NAME,
...
from models.ik_controller import (  # noqa: E402
    IKConfig,
...
def _draw_overlay_boxes(
    ...
    *,
    ...
    constraints_h = 0.165
    left_x = 0.055
    right_x = 1.0 - left_x - box_w
    bottom = 0.065

    for (bx, by, bw, bh) in [(left_x, bottom, box_w, legend_h), (right_x, bottom, box_w, constraints_h)]:
        ax.add_patch(
            FancyBboxPatch(
                (bx, by),
                bw,
    ...
    ax.text(txt_x, row0, "Demonstrations", fontsize=15.5, va="center", color=text_rgb, transform=ax.transAxes)
    ...
    ax.text(txt_x, y2, "Orientation", fontsize=15.5, va="center", color=text_rgb, transform=ax.transAxes)

    y3 = row0 - 2 * row_gap
    ax.add_patch(Circle((sym_x + 0.02, y3), 0.0085, transform=ax.transAxes, color=sample_rgba[:3], alpha=0.95))
    ax.text(txt_x, y3, "Local perturbed samples", fontsize=15.5, va="center", color=text_rgb, transform=ax.transAxes)

    # Constraints box
    c_y = bottom
    ax.text(right_x + 0.025, c_y + constraints_h - 0.034, "Training constraints loss", fontsize=16.2, fontweight="semibold",
            color=text_rgb, transform=ax.transAxes)
    csym_x = right_x + 0.035
    ctxt_x = right_x + 0.095
    crow0 = c_y + constraints_h - 0.076
    crow_gap = 0.038

    ax.plot([csym_x, csym_x + 0.04], [crow0, crow0], color=traj_rgba[:3], linewidth=5.0,
            transform=ax.transAxes, solid_capstyle="round")
    ax.text(ctxt_x, crow0, r"$h(x_{\mathrm{demo}})=0$", fontsize=15.8, va="center",
            color=text_rgb, transform=ax.transAxes)

    cy2 = crow0 - crow_gap
    ax.add_patch(Circle((csym_x + 0.02, cy2), 0.0085, transform=ax.transAxes, color=sample_rgba[:3], alpha=0.95))
    ax.text(ctxt_x, cy2, r"$\|\nabla h_i(x)\|=1$", fontsize=15.8, va="center",
            color=text_rgb, transform=ax.transAxes)

    cy3 = crow0 - 2 * crow_gap
    ax.add_patch(Circle((csym_x + 0.02, cy3), 0.0085, transform=ax.transAxes, color=sample_rgba[:3], alpha=0.95))
    ax.text(ctxt_x, cy3, r"$\nabla h_i(x)^\top \nabla h_j(x)=0,\ i\neq j$", fontsize=15.2, va="center",
            color=text_rgb, transform=ax.transAxes)
